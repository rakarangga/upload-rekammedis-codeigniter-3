...
class Dashboard extends Admin_Controller
{
    ...
    //   $this->data['getPickup'] = $this->International_m->getUncprocess('2');
      ...
      $this->data['year'] = $this->Directory_m->get_dropdown_tahun();
    //   dump($this->data['year']);
    ...
    public function grafikbatang()
    {
        ...
        $year =  $this->Directory_m->get_dropdown_tahun();
    ...
    public function grafikpie()
    {
        $thn = $this->input->post('thn');
        $thn || $thn = encrypting(date('Y'));
        $querys = $this->Directory_m->get_by(array(
            'YEAR(t_directory.tgldirectory)' => decrypting($thn),
        ));
		...
		if (count($querys)) {
            ...
            $arr[str_replace('""', '', "label")] = tgl_bulan($query->tgldirectory);
    ...
    //    dump(json_encode($collection));
         $this->data['color'] = json_encode(implode(',',$color));
